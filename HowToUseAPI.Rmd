...
If we want detail, we can get a data frame containing the services and short description of each. 
```{r}
...
Now that we have the call, we can request data from the API. The perform.call() function will provide a data frame with info about the request and data desired.
...
```
We can opt to just see the variable names as follows:
```{r}
show.variables()
```
