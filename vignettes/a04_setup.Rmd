...
You'll need to acquire an API key from AQS. To do so, `epair` provides the function `get.aqs.key()` with your email as its parameter. After calling `get.aqs.key()` check your email to obtain your key. `epair` uses `.Renviron` to access your email and key when making calls. A simple way to setup your key and email is through the `usethis` package with the following command. 
